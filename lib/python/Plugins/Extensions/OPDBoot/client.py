        ...
        def handleResponse(self, response):
                if self.quietLoss:
                        ...
                        if self.factory.method == 'HEAD':
                                ...
                                self.factory.noPage(failure.Failure(PartialDownloadError(self.status, self.message, response)))
                        else:
...
class HTTPPageDownloader(HTTPPageGetter):
        ...
        def handleResponseEnd(self):
                if self.length:
                        ...
                        self.factory.noPage(failure.Failure(PartialDownloadError(self.status)))
                if self.transmittingPage:
                        self.factory.pageEnd()
                        self.transmittingPage = 0
                if self.failed:
                        self.factory.noPage(failure.Failure(error.Error(self.status, self.message, None)))
                        self.transport.loseConnection()
                return
...
class Agent(_AgentBase):
        ...
        def request(self, method, uri, headers = None, bodyProducer = None):
                ...
                try:
                        endpoint = self._getEndpoint(parsedURI.scheme, parsedURI.host, parsedURI.port)
